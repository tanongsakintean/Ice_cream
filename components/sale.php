...
if (isset($_REQUEST['ac']) == "sec") {
    if ($_REQUEST['keyword'] != '') {
        $val = $meu->qty("SELECT * FROM tb_member WHERE tb_member.mem_status = 2 AND (tb_member.mem_username LIKE '%" . $_REQUEST['keyword'] . "%' OR tb_member.mem_firstname LIKE '%" . $_REQUEST['keyword'] . "%' OR tb_member.mem_lastname LIKE '%" . $_REQUEST['keyword'] . "%' OR tb_member.mem_phone LIKE '%" . $_REQUEST['keyword'] . "%')  ");
    } else {
        $val = $meu->qty("SELECT * FROM tb_member WHERE mem_status = 2");
    }
} else {
    $val = $meu->qty("SELECT * FROM tb_member WHERE mem_status = 2");
}
...
<div class="container-fluid text     p-5 bg-white">
    <div>
        <button data-toggle="modal" data-target="#addmodal" class="text-white text-lg  px-3 py-2 rounded-md bg-green-500 hover:bg-green-600 "><i class="fas fa-plus text-white"></i> เพื่มข้อมูลพนักงาน</button>
    </div>
    <div class="  mt-4 h-2 " style="background-color: #ECF2F5;"></div>
    <div class="my-4">
        <form action="?p=sale&ac=sec" method="post">

            <div class="mt-4 mb-3 ">
                <h1 class="text-lg ">ค้นหาพนักงาน</h1>
            </div>
            <div class="my-3">
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header p-4">
                    <h5 class="text-xl " id="exampleModalLongTitle">เพิ่มข้อมูลพนักงาน</h5>
                    <i data-dismiss="modal" aria-label="Close" class="cursor-pointer text-red-500 text-xl hover:text-red-600 fas fa-window-close"></i>
                </div>
                <form action="action/user_db.php?ac=new_sale" method="POST" enctype="multipart/form-data" onSubmit="return ajaxSubmit(this)">
    ...
    <div class="modal fade bd-example-modal-lg" id="editmodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header p-4">
                    <h5 class="text-xl ">แก้ไขข้อมูลพนักงาน</h5>
                    <i data-dismiss="modal" aria-label="Close" class="cursor-pointer text-red-500 text-xl hover:text-red-600 fas fa-window-close"></i>
                </div>
